...
function AllNotes(){
    ...
    async function fetchSubject(){
        ...
        try{
            ...
            const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/admin/getType`, {params: queryParams})
    ...
    async function fetchUnits(){
        try{
            ...
            const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/admin/subject`, {params: queryParams})
    ...
    return (
        <div className="d-inline-flex flex-column">
            <h1>Notes</h1>
            {selectedSubjectId  ? 
                <ol>All units for {selectedSubjectName} 
                    {units.length > 0 ? (
                    units.map((i, n) => (
  <li key={n}>
    <a href={i.fileUrl} target="_blank" rel="noopener noreferrer">
      {i.unitName}
            ...
            : (
                ...
                <ol>
                {subjects.length > 0 ? (
                    subjects.map((i,n) => <li key={n} onClick={() => selectedSubject(i)} >{i.subjectName}</li>)
                ) : (
                    <li>No Subject Found</li>
                )}
            </ol>
